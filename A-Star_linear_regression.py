...
def getHeuristicCost(board):
    q_weights = get_Q_Weight(board)
    total_weights = np.sum(q_weights)
    heaviest_queens = np.max(q_weights)
    lightest_queens = np.min(q_weights)
    atck = list()
    qw = sorted(q_weights)
    for i in qw:
        atck.append(getQueenAttacks(i,init_board))
    heaviest_queen_attacks = atck[-1]
    if atck[0] == 0:
        heaviest_lightest_attack_ratio = 0
    else:
        heaviest_lightest_attack_ratio = atck[-1]/atck[0]
    
    MLheu = (9.6141 * total_weights) + (-7.495 * heaviest_queens) + (-3.5481 * lightest_queens) +  (34.8749 * heaviest_lightest_attack_ratio) + (10.8857 * heaviest_queen_attacks) + (-111.8746)
    return MLheu
...
toc = time()
print(f"The time taken to run {boards} iterations with linear regression heuristic: {int(toc - tic)} secs.")
...
df1 = pd.DataFrame.from_dict({"Initial_Boards":initial_boards, "Final_Boards":final_boards, "Times Taken":times_taken, "Branching Factor":branching_factors, "Nodes/sec":nodes_time, "Cost":final_costs})
df1.to_csv('linear_reg_heuristic_data.csv',header = True, index = False)
