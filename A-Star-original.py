...
def getHeuristicCost(board):
    sum = 0
    q_wts = get_Q_Weight(board)
    for i in range(len(q_wts)):
        queenAttacks = getQueenAttacks(q_wts[i], board)
        sum += q_wts[i]**2 * queenAttacks
    return sum
...
while j < boards:
    ...
    if attacking_queen_pair == 0:
        ...
        if t == 0:
            nodes_per_sec = number_of_expanded_states
        else:
...
toc = time()
print(f"The time taken to run {boards} iterations with original heuristic: {int(toc - tic)} secs.")
...
df1.to_csv('original_heuristic_data.csv',header = True, index = False)
